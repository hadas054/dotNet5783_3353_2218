    ...
    private static void simulationProgress()
    {
        ...
        while (!s_flagStopSimulation)
        {
            ...
            else
            {
                ...
                order = order.status == OrderStatus.Paid ? _bl.Order.SentOrder(order.Id)
                        : _bl.Order.DeliveredOrder(order.Id);

                items =
                     Tuple.Create(order, (order.status == OrderStatus.Shipped ? (OrderStatus?)OrderStatus.Delievered : null), "", "");
